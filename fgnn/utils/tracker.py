...
class WandBTracker:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        tracker_resume = "allow" if resume else None
    ...
    def log_scalars(self, tag_scalar_dict: dict, global_step: int = None):
    ...
    def log_video(
        ...
        if video.ndim > 4:
            for index, vid in enumerate(video):
                self.log_video(tag=f"{tag}_{index}", video=vid, global_step=global_step)
        else:
    ...
    def log_plot(self, tag: str, values: pd.DataFrame, xtitle, ytitle, classes_marker):
        table = wandb.Table(dataframe=values)
        plt = wandb.plot.line(
            table,
            x=xtitle,
            y=ytitle,
            stroke=classes_marker,
            title=tag,
        )
        if self.context:
    ...
    def log_file(self, file_name: str = None):
        wandb.save(
    ...
    def log_checkpoint(self, tag: str, state_dict: dict, global_step: int = None):
...
def wandb_experiment(params: dict, logger=None, reinit=False) -> WandBTracker:
    ...
    wandb_logger.log_tags(tracker_params.get("tags", ()))
